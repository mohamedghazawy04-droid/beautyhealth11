...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-xs flex justify-end animate-in fade-in duration-200 text-right">
      <div className="w-full max-w-md bg-white h-full flex flex-col shadow-2xl border-l border-pink-100">
        {/* Drawer Header */}
        <div className="p-4 border-b border-pink-100 flex items-center justify-between bg-pink-50/50">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-pink-600 to-rose-600 text-white flex items-center justify-center shadow-xs">
              <ShoppingBag className="w-5 h-5" />
            </div>
            <div>
              <h2 className="font-extrabold text-stone-900 text-base">سلة مشترياتك</h2>
              <p className="text-xs text-stone-500">
                {items.length} {items.length === 1 ? 'منتج' : 'منتجات مختارة'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-3 bg-pink-50 border-b border-pink-100 text-xs">
          <div className="flex items-center justify-between text-pink-950 font-bold mb-1.5">
            <span className="flex items-center gap-1.5">
              <Truck className="w-3.5 h-3.5 text-pink-600" />
              {isFreeDelivery
                ? '🎉 تهانينا! التوصيل مجاني لطلبك'
                : `أضيفي بـ ${amountNeededForFreeShipping} ج للحصول على شحن مجاني!`}
            </span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {items.length === 0 ? (
            <div className="text-center py-16 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500 mt-1 max-w-xs mx-auto">
                  تصفحي المنتجات وأضيفي ما يناسبك إلى السلة لإتمام الطلب والتوصيل الفوري.
                </p>
              </div>
            </div>
          ) : (
            items.map((item) => (
              <div
                key={item.product.id}
                className="p-3 rounded-2xl border border-pink-100 bg-white shadow-2xs flex items-center gap-3"
              >
                <img
                  ...
                  className="w-16 h-16 object-cover rounded-xl border border-pink-50 shrink-0"
                />
                ...
                <div className="flex flex-col items-end justify-between h-full gap-2">
                  ...
                  <div className="flex items-center gap-1.5 bg-pink-50/60 px-1.5 py-0.5 rounded-lg border border-pink-200/60">
                    <button
                      onClick={() => onUpdateQuantity(item.product.id, -1)}
                      className="w-5 h-5 rounded flex items-center justify-center font-bold text-stone-700 hover:bg-pink-200/60 cursor-pointer"
                    >
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-pink-100 bg-pink-50/30 space-y-3">
            {/* Coupon Code Section */}
            <div>
              {appliedCoupon ? (
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-pink-100 text-pink-900 text-xs font-bold border border-pink-300">
                  <div className="flex items-center gap-1.5">
                    <Tag className="w-3.5 h-3.5 text-pink-700" />
                ...
                <form onSubmit={handleApplyCoupon} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 rounded-xl bg-white border border-pink-200 text-xs uppercase font-mono focus:ring-2 focus:ring-pink-500 focus:outline-none"
                  />
                  <button
                    type="submit"
                    className="px-3.5 py-1.5 bg-stone-800 hover:bg-stone-900 text-white font-bold rounded-xl text-xs transition-colors cursor-pointer"
                  >
                    تطبيق
                  </button>
                </form>
              ...
              {couponFeedback && (
                <div
                  className={`text-[11px] mt-1 flex items-center gap-1 font-semibold ${
                    couponFeedback.success ? 'text-pink-700' : 'text-rose-600'
                  }`}
                >
            ...
            <div className="space-y-1.5 text-xs text-stone-600 pt-1 border-t border-pink-100">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-stone-900">
                  {effectiveDeliveryFee === 0 ? (
                    <span className="text-pink-700 font-black">مجاني 🎉</span>
                  ) : (
              ...
              <div className="flex justify-between text-sm font-black text-stone-950 pt-2 border-t border-pink-200">
                <span>الإجمالي النهائي:</span>
                <span className="text-base text-pink-700 font-black">{grandTotal} جنيه</span>
              </div>
            </div>
