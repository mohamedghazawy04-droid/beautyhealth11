...
export const WishlistModal: React.FC<WishlistModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs text-right">
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-stone-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3">
          {wishlist.length === 0 ? (
            ...
            wishlist.map((product) => (
              <div
                key={product.id}
                className="p-3.5 rounded-2xl border border-stone-200 bg-white shadow-2xs flex items-center gap-3 justify-between"
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="px-3 py-2 bg-emerald-800 hover:bg-emerald-900 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
                  >
